...
The input and output of runMBASED() function are SummarizedExperiment
objects, defined in R package 'GenomicRanges'. Please see the
SummarizedExperiment help page for details. Briefly, the input object
...
SummarizedExperiment object, with rows corresponding to individual
aseIDs and a single column. MBASED output at the level of individual
SNV is stored in the exptData of the output object. The examples
...
rowData(ASEresults_1s_haplotypesKnown)
names(exptData(ASEresults_1s_haplotypesKnown))
class(exptData(ASEresults_1s_haplotypesKnown)$locusSpecificResults)
names(assays(exptData(ASEresults_1s_haplotypesKnown)$locusSpecificResults))
assays(exptData(ASEresults_1s_haplotypesKnown)$locusSpecificResults)$allele1IsMajor
assays(exptData(ASEresults_1s_haplotypesKnown)$locusSpecificResults)$MAF
rowData(exptData(ASEresults_1s_haplotypesKnown)$locusSpecificResults)
...
The output of \texttt{runMBASED()} is a SummarizedExperiment object
...
since this gene
only has 1 tested SNV. exptData slot of the return object contains a
single element, `locusSpecificResults', which is a
SummarizedExperiment object with information on MBASED results at the
...
isoSpecificExample <- SummarizedExperiment(
  ...
  rowData=isoSpecificExampleSNVs
)
...
rowData(ASEresults_2s)
names(exptData(ASEresults_2s))
class(exptData(ASEresults_2s)$locusSpecificResults)
names(assays(exptData(ASEresults_2s)$locusSpecificResults))
assays(exptData(ASEresults_2s)$locusSpecificResults)$allele1IsMajor
assays(exptData(ASEresults_2s)$locusSpecificResults)$MAFDifference
rowData(exptData(ASEresults_2s)$locusSpecificResults)
...
analysis is a SummarizedExperiment object
...
SummarizedExperiment object. By default, MBASED assumes that the background (no ASE) allelic
ratio is 50/50:
